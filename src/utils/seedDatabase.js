/**
 * Database Seeding Utility for React Native
 * 
 * This is a React Native-compatible version of the seeding script.
 * Can be called from a dev/debug screen or component.
 * 
 * Usage:
...
const testItems = [
  ...
  },
  {
    ...
    category: 'Services',
    condition: 'N/A',
    images: ['https://images.unsplash.com/photo-1542038784456-1ea8e935640e?w=500'],
    userId: 'test_user_5',
...
export async function seedDatabase() {
  ...
  try {
    ...
    // 6. Seed Conversations with Messages
...
async function createMatch(user1Id, user2Id, item1Id, item2Id) {
  const user1 = testUsers.find(u => u.uid === user1Id);
  const user2 = testUsers.find(u => u.uid === user2Id);
  const item1 = testItems.find(i => i.userId === user1Id);
  const item2 = testItems.find(i => i.userId === user2Id);
  ...
  const matchData = {
    ...
    user1ItemId: item1Id,
    user2ItemId: item2Id,
    user1ItemTitle: item1?.title || 'Item 1',
    user2ItemTitle: item2?.title || 'Item 2',
    status: 'matched',
    createdAt: serverTimestamp(),
  ...
  const conversationData = {
    ...
    participants: [user1Id, user2Id],
    participantNames: {
      [user1Id]: user1?.name || 'User 1',
      [user2Id]: user2?.name || 'User 2'
    },
    participantPhotos: {
      [user1Id]: null,
      [user2Id]: null
...
export { testUsers, testItems, testServices };
